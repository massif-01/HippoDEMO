...
class SystemAudioCapture: NSObject, SCStreamOutput, SCStreamDelegate, SCContentSharingPickerObserver {
    ...
    init(outputPath: String, usePicker: Bool = true) {
    ...
    func start() async throws {
        if !usePicker {
            ...
            try await beginCapture(with: filter)
            return
        }
...
func printUsage() {
    ...
    USAGE:
        ownscribe-audio capture --output FILE [--display] [--mic] [--mic-only] [--mic-device NAME] [--silence-timeout N]
        ownscribe-audio list-apps
        ownscribe-audio list-devices

    OPTIONS:
        --output, -o FILE    Output WAV file path (required for capture)
        --display            Capture the first display without showing the content picker
        --mic                Also capture microphone input
        --mic-only           Capture microphone input only, without system audio
...
func main() {
    ...
    case "capture":
        ...
        // Determine paths: if system + mic is enabled, use temp files then merge.
        // In mic-only mode, write the mic recording directly to the requested output.
        let systemPath = enableMic && !micOnly ? output + ".sys.tmp.wav" : output
        let micPath = micOnly ? output : output + ".mic.tmp.wav"

        let capture = micOnly ? nil : SystemAudioCapture(outputPath: systemPath, usePicker: usePicker)
            ...
            if let mic = micCapture {
                ...
                if !micOnly {
                    guard let capture = capture else {
                        fputs("Error merging audio: missing system capture\n", stderr)
                        exit(1)
                    }
                    do {
                        try mergeAudioFiles(
