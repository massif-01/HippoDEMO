...
class CoreAudioRecorder(AudioRecorder):
    ...
    def start(self, output_path: Path) -> None:
        ...
        mic_only = os.environ.get("OWNSCRIBE_AUDIO_MIC_ONLY", "").lower() in {"1", "true", "yes", "on"}
        if (
            not mic_only
            and os.environ.get("OWNSCRIBE_AUDIO_DISPLAY", "").lower() in {"1", "true", "yes", "on"}
        ):
            cmd.append("--display")
        if mic_only:
            cmd.append("--mic-only")
        elif self._mic or self._mic_device:
            cmd.append("--mic")
        if self._mic_device:
